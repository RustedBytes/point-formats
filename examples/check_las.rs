...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Scan Complete! Stats from {} scanned points:",
        actual_point_count
    );
    println!(
        "  Points with Intensity: {} ({:.1}%)",
        intensity_count,
        (intensity_count as f64 / actual_point_count as f64) * 100.0
    );
    println!(
        "  Points with Color:     {} ({:.1}%)",
        color_count,
        (color_count as f64 / actual_point_count as f64) * 100.0
    );
    println!(
        "  Points with GPS Time:  {} ({:.1}%)",
        gps_time_count,
        (gps_time_count as f64 / actual_point_count as f64) * 100.0
    );
    ...
    for (class_code, count) in &classification_stats {
        ...
        println!(
            ...
            name,
            count,
            (*count as f64 / actual_point_count as f64) * 100.0
        );
    }
